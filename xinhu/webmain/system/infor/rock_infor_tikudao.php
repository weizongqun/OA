...
$(document).ready(function(){
	var c={
		yulan:function(){
			...
			s+='<tr><td></td><td>题名</td><td>分类</td><td>类型</td><td>答案A</td><td>答案B</td><td>答案C</td><td>答案D</td><td>答案</td><td>说明</td></tr>';
			a = cont.split('\n');
		...
		downss:function(){
			js.open('upload/base/tikuimport.xls');
		},
		insrtss:function(){
			var val = mobjs.val();
			mobjs.val(val+'	');
		...
		saveadd:function(){
			...
			js.setmsg('处理中...','', vis);
			js.ajax(js.getajaxurl('tikuimport','{mode}','{dir}'),{importcont:val},function(ds){
				if(ds.success){
					js.msg('success', ds.msg);
					closenowtabs();
					try{tikumangena.reload();}catch(e){}
				}else{
...
<div align="left">
<div>请根据Excel格式添加数据，分类格式如：[技术姿势/PHP知识]，将数据复制到下面文本框中，<a click="downss" href="javascript:;">下载导入Excel表格格式</a><br>也可以手动输入，多行代表多记录，整行字段用	分开，<a click="insrtss" href="javascript:;">插入间隔符</a></div>

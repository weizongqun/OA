...
class reimClassModel extends Model
{
	...
	*/
	public function installwx($lx=0)
	{
		if($lx==0){
			...
			$bo = false;
			if($this->optiondb->getval('weixinqy_corpid')!='')$bo=true;
			return $bo;
		}
	}
	...
	*/
	public function getalias($uid, $lx=0)
	{
		...
		$rows 	= m('logintoken')->getrows("`uid` in(select id from `[Q]admin` where $uwhere) and `cfrom` in ('appandroid','appios') and `online`=1",'`token`,`uid`','id desc');
		$alias 	= $uida = array();
		$uids	= '0';
		foreach($rows as $k=>$rs){
			$_uid 	 = $rs['uid'];
	...
	*/
	...
	{
		$garr = $this->getalias($receid, $lx);
		if(!$garr)return false;
		$alias	= $garr['alias'];
		if(!$alias)return false;
		$uids	= $garr['uids'];
		$cont	= '';
		foreach($conta as $k=>$v)$cont.=',"'.$k.'":"'.$v.'"';
		$cont 	= '{'.substr($cont,1).'}';
		
		return c('JPush')->push($title, $cont, $alias, $uids);

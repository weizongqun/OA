...
class inputChajian extends Chajian
{
	...
	{
		$this->adminid 	= $uid;
		$this->urs  	= m('admin')->getone($uid, '`name`,`deptname`,`ranking`,`deptid`,`num`,`deptallname`,`tel`,`mobile`,`email`,`id`,`superman`');
	...
	*	读取表单样式(有默认值的)
	*/
	public function getfieldcontval($fid, $val=false)
	{
		return $this->getfieldcont($fid, null,'',0, $val);
	}
	...
	*/
	public function getfieldcont($fid, $objs=null, $leox='', $iszb=0, $deval=false)
	{
		...
		if($val!=''){
			...
			if(substr($val,0,5)=='{urs.')$val  = $this->rock->arrvalue($this->urs, substr($val,5,-1));
